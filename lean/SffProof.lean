-- SffProof: SCFF gradient-alignment-at-initialization formalization.
-- Layered per design.md §3: Defs (0), Skeleton (1), Hypotheses (2), Main (3).
import SffProof.Defs
import SffProof.Skeleton
import SffProof.Hypotheses
import SffProof.Main
import SffProof.Softmax
import SffProof.Normalization
import SffProof.Residual
import SffProof.InfoBound
import SffProof.Probability.Moments
import SffProof.Probability.RandomMatrix
import SffProof.Probability.Subspace
